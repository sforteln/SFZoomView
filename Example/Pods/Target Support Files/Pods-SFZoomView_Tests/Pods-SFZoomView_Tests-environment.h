
// To check if a library is compiled with CocoaPods you
// can use the `COCOAPODS` macro definition which is
// defined in the xcconfigs so it is available in
// headers also when they are imported in the client
// project.


// FBSnapshotTestCase
#define COCOAPODS_POD_AVAILABLE_FBSnapshotTestCase
#define COCOAPODS_VERSION_MAJOR_FBSnapshotTestCase 1
#define COCOAPODS_VERSION_MINOR_FBSnapshotTestCase 8
#define COCOAPODS_VERSION_PATCH_FBSnapshotTestCase 1

// SFZoomView
#define COCOAPODS_POD_AVAILABLE_SFZoomView
#define COCOAPODS_VERSION_MAJOR_SFZoomView 0
#define COCOAPODS_VERSION_MINOR_SFZoomView 1
#define COCOAPODS_VERSION_PATCH_SFZoomView 0

